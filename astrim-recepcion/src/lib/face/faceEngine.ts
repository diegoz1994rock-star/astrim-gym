...
/**
 * Único módulo que importa face-api. AccessScreen nunca toca face-api
 * directamente — así el motor se puede cambiar sin tocar la UI. La
 * comparación contra rostros enrolados NUNCA ocurre aquí: este archivo
 * solo detecta y calcula el embedding local; el matching es responsabilidad
 * del servidor (ver lanClient.registerAccessByFace).
 */
...
export interface FaceDetectionResult {
  ...
  /** Embedding de 128 floats, listo para enviar al servidor. Solo si quality === "GOOD". */
  descriptor: number[] | null;
}

/**
 * Rango de tamaño "ideal" del rostro (ancho relativo al frame). Fuera de él
 * la calidad se reporta como TOO_FAR / TOO_CLOSE para guiar al usuario. Es
 * más ancho que antes: distintas cámaras encuadran muy diferente.
 */
...
export interface DetectFaceOptions {
  /**
   * "goodSizeOnly" (por defecto): embedding solo con buen tamaño (enrolar).
   * "always": embedding con cualquier rostro detectado (reconocer — el
   * servidor filtra por distancia).
   */
...
export async function detectFace(
  ...
  const descriptor =
    descriptorMode === "always" || quality === "GOOD" ? Array.from(result.descriptor) : null;
